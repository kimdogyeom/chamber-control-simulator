...
이 프로젝트는 C# WinForms로 만든 **가상 열처리 챔버 제어 시뮬레이터**다. 화면 동작을 우선하는 예제가 아니라, 상태 전이와 안전 조건을 UI에서 분리해 테스트 가능한 규칙으로 다루는 데 초점을 둔다.
...
핵심은 `ThermalController`가 안전 상태의 최종 판단 권한을 가진다는 점이다. Form이나 Presenter가 조건을 해석해 Alarm을 임의로 해제하지 않는다.
...
따라서 Core의 온도 변화·Holding 누적·SensorTimeout은 `TimeSpan` 기준으로 계산된다. `Thread.Sleep`, UI thread block, `while` 기반 UI 제어 루프는 사용하지 않는다.
...
Alarm 중 Stop 요청은 안전 상태를 우회하지 못한다. Reset 역시 Alarm을 즉시 지우는 동작이 아니라, Controller가 Recovery-ready라고 판단한 경우에만 Idle로 복귀하는 마지막 전이다.
