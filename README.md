...
| P3-T3 Core plant simulation 분리 | Planned | legacy `ThermalController.Tick`의 synthetic temperature 변경 제거 예정 |
...
P3-T4 전에는 위 Application path가 WinForms runtime composition root에 연결되지 않는다. P4 전에는 UI Start/Stop/Reset을 PLC command write와 semantic ACK로 연결했다고 주장하지 않는다.
...
P3-T2는 Core-owned `ThermalObservation`으로 external input을 적용한다 (`3a7398d`). 하지만 legacy UI의 `Tick`이 synthetic temperature를 바꾸는 책임은 P3-T3에서 제거할 예정이므로, 아직 Core가 plant simulation과 완전히 분리됐다고 주장하지 않는다.
...
- P1/P2/P3-T1의 source/test/review provenance는 각 local commit과 independent review bundle에 남아 있다. P3-T2 source verification은 [`docs/verification/p3-t2-atomic-observation.md`](docs/verification/p3-t2-atomic-observation.md)에 `3a7398d` SHA와 함께 기록한다.
- P3-T2의 65/65는 `3a7398d` source commit에 bound된 verification result다. 다만 P3-T3/T4/P4의 completion evidence나 release evidence 전체를 뜻하지 않는다.
